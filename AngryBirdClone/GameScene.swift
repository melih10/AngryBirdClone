...
//  AngryBirdClone
//
//  Created by Melih on 18.11.2022.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        bird2.size = CGSize(width: self.frame.width / 18 , height: self.frame.height / 10)
        bird2.zPosition = 1
        self.addChild(bird2)
        */
                    ...
                    for node in touchNodes {
                        if let sprite = node as? SKSpriteNode {
                            if sprite == bird {
                                ...
                                let dx = -5 * (touchLocation.x - originalPosition!.x)
